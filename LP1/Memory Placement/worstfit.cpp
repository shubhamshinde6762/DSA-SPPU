    ...
    void worstFit()
    {
        while (!jq.empty())
        {
            ...
            for (int i = 0; i < mq.size(); i++)
            {
                ...
                {
                    if (mq[i]->blockSize > maxSize)
                    {
                        maxSize = mq[i]->blockSize;
                        worstIdx = i;
                    }
                }
            }
            ...
            if (worstIdx != -1)
            {
                mq[worstIdx]->status = true;
                cout << "Allocated Memory Block at Location " << mq[worstIdx]->memoryLocation
                     << " of Size " << mq[worstIdx]->blockSize << " to Job Id " << job->jobId << endl;
            }
            else
            {
